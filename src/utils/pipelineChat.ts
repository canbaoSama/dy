...
export function buildPipelineStageBody(completedStageKey: string, d: JobDetailResponse): string {
  ...
  switch (completedStageKey) {
    ...
    case 'extracting_content': {
      ...
      if (n != null && n > 0) {
        lines.push(`步骤 1/2：正文抽取完成（约 ${n} 字）。`)
      } else {
        lines.push('步骤 1/2：正文抽取完成（正文不足时已使用摘要降级）。')
      }
      if (d.news_title) lines.push(`当前稿件：${d.news_title.slice(0, 120)}${d.news_title.length > 120 ? '…' : ''}`)
      lines.push('下一步：即将展示素材候选，请选择后继续。')
      break
    }
    ...
    case 'generating_subtitles': {
      ...
      if (Array.isArray(cues) && cues.length) {
        lines.push(`字幕草稿已生成（共 ${cues.length} 条，可在对话框编辑确认）。`)
      } else {
        lines.push('字幕草稿已生成，可在对话框编辑确认。')
      }
      break
    }
    ...
    case 'ready_for_review':
      lines.push('流水线已全部完成，成片已在对话框内可直接预览。')
      break
    default:
